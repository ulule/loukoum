...
func TestParseJoin(t *testing.T) {
	...
	// With logical operator
	{
		...
		is.Equal("project.user_id", on.Right.Name)
		is.Empty(on.Right.Alias)
		operator, ok := infix.Operator.(stmt.LogicalOperator)
		is.True(ok)
		is.Equal(types.And, operator.Operator)
		on, ok = infix.Right.(stmt.On)
		is.True(ok)
		is.NotEmpty(on)
	...
	}
	{
		...
		operator, ok := infix.Operator.(stmt.LogicalOperator)
		is.True(ok)
		is.Equal(types.Or, operator.Operator)
		on, ok = infix.Right.(stmt.On)
		is.True(ok)
		is.NotEmpty(on)
		is.Equal("user.hash", on.Left.Name)
	...
	}
	{
		...
		is.Equal("project.user_id", on.Right.Name)
		is.Empty(on.Right.Alias)
		operator, ok := infix2.Operator.(stmt.LogicalOperator)
		is.True(ok)
		is.Equal(types.And, operator.Operator)
		on, ok = infix2.Right.(stmt.On)
		is.True(ok)
		is.NotEmpty(on)
		is.Equal("user.hash", on.Left.Name)
		is.Empty(on.Left.Alias)
		is.Equal("project.hash", on.Right.Name)
		is.Empty(on.Right.Alias)
		operator, ok = infix1.Operator.(stmt.LogicalOperator)
		is.True(ok)
		is.Equal(types.Or, operator.Operator)
		on, ok = infix1.Right.(stmt.On)
		is.True(ok)
		is.NotEmpty(on)
